...
# NOTE: update these paths once your desert .glb files are in place under
# kenney_tower-defense-kit/Models/GLB format/desert/
var tile_base = load("res://kenney_tower-defense-kit/Models/GLB format/desert/tile-dirt.glb")
var tile_straight = load("res://kenney_tower-defense-kit/Models/GLB format/desert/desert-tile-straight.glb")
var tile_corner = load("res://kenney_tower-defense-kit/Models/GLB format/desert/desert-tile-corner-round.glb")
var tile_spawn = load("res://kenney_tower-defense-kit/Models/GLB format/desert/desert-tile-spawn-round.glb")
var tile_end = load("res://kenney_tower-defense-kit/Models/GLB format/desert/desert-tile-end-round.glb")

# Desert decoration props (swap in for the snow biome's tree/tree-large/rock)
var cactus_model = load("res://kenney_tower-defense-kit/Models/GLB format/desert/desert-cactus.glb")
var cactus_large_model = load("res://kenney_tower-defense-kit/Models/GLB format/desert/desert-cactus-large.glb")
var tumbleweed_model = load("res://kenney_tower-defense-kit/Models/GLB format/desert/desert-tumbleweed.glb")
...
var spawner_script = preload("res://Desert-Stage/Spawner.gd")

# Batching containers for repeated static meshes
var mm_base: MultiMeshInstance3D
var mm_cactus: MultiMeshInstance3D
var mm_cactus_large: MultiMeshInstance3D
var mm_tumbleweed: MultiMeshInstance3D
...
func _build_map():
	...
	mm_cactus = _make_multimesh_node("Cactus", cactus_model)
	mm_cactus_large = _make_multimesh_node("CactusLarge", cactus_large_model)
	mm_tumbleweed = _make_multimesh_node("Tumbleweeds", tumbleweed_model)

	var base_transforms: Array[Transform3D] = []
	var cactus_transforms: Array[Transform3D] = []
	var cactus_large_transforms: Array[Transform3D] = []
	var tumbleweed_transforms: Array[Transform3D] = []
		...
		for z in range(MAP_SIZE):
			...
			if path_lookup.has(pos):
				...
				if randf() > 0.8:
					...
					if r > 0.6:
						cactus_transforms.append(deco_xform)
					elif r > 0.3:
						cactus_large_transforms.append(deco_xform)
					else:
						tumbleweed_transforms.append(deco_xform)

	_apply_multimesh(mm_base, base_transforms)
	_apply_multimesh(mm_cactus, cactus_transforms)
	_apply_multimesh(mm_cactus_large, cactus_large_transforms)
	_apply_multimesh(mm_tumbleweed, tumbleweed_transforms)
